...
int randomIntegerBetweenTwoNumbers(int min, int max)
{
...
public:
    ...
    float startACO(bool performGU)
    {
        ...
        for (int ant = 0; ant < ANT_COUNT; ant++)
        {
            ...
            pair<float, vector<int>> result_path = tour(visited,path);
            float antTourDistance = result_path.first;
            vector<int> antTourPath = result_path.second;
        ...
        if (optimalDistance < bestDistance)
        {
            bestDistance = optimalDistance;
            bestPath = optimalPath;
        }
        // Local Optimization (if necessary)
        pair<float, vector<int>> path = localOptimization_2OPT(optimalPath, optimalDistance, 100);
        optimalDistance = path.first;
        optimalPath = path.second;
        if (performGU)
        {
    ...
    pair<float, vector<int>> localOptimization_2OPT(vector<int> optimalPath, float optimalDistance, int numIter)
    {
        int n = optimalPath.size();
        for(int iter=0; iter<numIter; iter++)
        {
            int idx1 = randomIntegerBetweenTwoNumbers(0, n-4);
            int idx2 = randomIntegerBetweenTwoNumbers(idx1+2, n-2);

            reverse(optimalPath.begin()+idx1+1, optimalPath.begin()+idx2+1);
            
            float newPathDistance = 0;
            for (int j = 0; j < n - 1; j++)
            {
                newPathDistance += edges[optimalPath[j]][optimalPath[j + 1]].distance;
            }
            
            if (newPathDistance > optimalDistance)
                reverse(optimalPath.begin()+idx1+1, optimalPath.begin()+idx2+1);
            else
                optimalDistance = newPathDistance;
        }

        return {optimalDistance, optimalPath};
    } 

    pair<float, vector<int>> tour(vector<bool> &visited, vector<int> &path)
    {
        float distance = 0.0;
        int currentCity = path.back();
        int firstCity = path[0];
        int visitedCitiesCount = path.size();

        if(visitedCitiesCount > 1)
        {
            for(int i=0; i<path.size()-1; i++)
            {
                distance += edges[path[i]][path[i+1]].distance;
            }
        }
