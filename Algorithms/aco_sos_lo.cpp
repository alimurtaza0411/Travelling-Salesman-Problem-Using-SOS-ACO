...
public:
    ...
    float startACO(bool performGU)
    {
        ...
        pair<float, vector<int>> path = localOptimization1_NN(optimalPath, optimalDistance);
    ...
    pair<float, vector<int>> localOptimization1_NN(vector<int> optimalPath, float optimalDistance)
    {
        int n = optimalPath.size();
        for (int i = 0; i < n - 2; i++)
        {
            int closestCity = i + 1;
            float closestCityDistance = edges[optimalPath[i]][optimalPath[i + 1]].distance;
            for (int j = i + 2; j < n - 1; j++)
            {
                if (edges[optimalPath[i]][optimalPath[j]].distance < closestCityDistance)
                {
                    closestCityDistance = edges[optimalPath[i]][optimalPath[j]].distance;
                    closestCity = j;
                }
            }

            reverse(optimalPath.begin() + i + 1, optimalPath.begin() + closestCity + 1);
    ...
    pair<float, vector<int>> localOptimization2(vector<int> optimalPath, float optimalDistance)
    {
        int n = optimalPath.size();
