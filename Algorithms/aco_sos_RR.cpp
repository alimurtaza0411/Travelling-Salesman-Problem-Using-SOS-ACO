...
public:
    ...
    float startACO(bool performGU)
    {
		...
		auto path = localOptimization_RR(optimalPath, optimalDistance,numCities);
    ...
    pair<float, vector<int>> localOptimization_RR(vector<int> &optimalPath, float optimalDistance, int numIter) 
    {
        for(int iter=0; iter<numIter; iter++)
        {
            ...
            path = {path.begin(), path.begin()+idx+1};
            vector<bool> visited(numCities, false);
            for(int city : path) 
            {
               visited[city] = true; 
            ...
            pair<float, vector<int>> newPath =tour(visited, path);
            if(newPath.first<optimalDistance)
            {
                optimalDistance = newPath.first;
                optimalPath = newPath.second;
            }
        }
